    ...
    if (isEditMode==NO) {
        ...
        if (activedButton != nil && [editNumButtons indexOfObject:activedButton]>99) { // Posible Value Mode
            if (isPosibleValueMode) {
                ...
                NSLog(@"bottom of if statement %@ == %@", sender.titleLabel.text, activedButton.titleLabel.text);
...
//            NSLog(@"Printing key value %@  rowcol %@", value, rowcol);
...
-(IBAction) solveButtonPressed:(UIButton *)sender {
    NSLog(@"solve button is pressed here");
...
- (IBAction)numButtonPressed:(UIButton *)sender {
    ...
    NSLog(@"after activedButton if statement");
    activedButton = sender;
    NSLog(@"the number is %@", activedButton.currentTitle);
...
- (IBAction)clearButtonPressed:(UIButton *)sender {
    NSLog(@"Am I awesome or what? ClearButtonNotAllPressed");
...
- (IBAction)clearAllButtonPressed:(UIButton *)sender {
    NSLog(@"Is this clear all button pressed?");
    if (nowMode != playMode) {
        editNumButtons = [[NSMutableArray alloc] initWithCapacity:81];
...
-(IBAction)editModeButtonPressed:(UIButton *)sender {
    NSLog(@"edit mode button pressed");
    [self notify];
    if (isEditMode == YES) {
        isEditMode = NO;
...
- (IBAction)createButtonPressed:(id)sender {
    ...
    NSLog(@"Create button was pressed");
    sudokuTable = [MySudokuClass initSudokuTable:sudokuTable];
...
- (IBAction) homeButtonPressed:(UIButton *)sender {
    NSLog(@"Home button was pressed just now");
    [self loadStartView];
}
